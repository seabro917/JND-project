% Final main algorithm with further bitrate checking.
function [QP_result, searching_level] = Main_function_final_checking(mode_flag,N,video_index,SUR_value,Bitrate_condition,pdf_model)
% Main function
...
for i = 1:N
    ...
    if (flag_outter == -1)
        ...
        % Meaning this point meet the bitrate condition, but and the SUR
        % condition at the same time, but what if we can move this point to
        % a even lower QP value? Meaning with given bitrate condition, we
        % can achieve a better SUR value than the given condition? This is
        % what we will have to check.
        QP_result_temp = QP_result_temp - 1;
        flag_inner = Compare_bitrate(bitrate_matrix_used_for_comparison(QP_result_temp-7),Bitrate_condition);
        while (flag_inner == 1)
            %  Case where the bitrate requirment is too big, the algorithm found the QP of 0.
            if (QP_result_temp == 8 ) % Thinking about the array indice issue.
                break;
            end
            QP_result_temp = QP_result_temp - 1;
            flag_inner = Compare_bitrate(bitrate_matrix_used_for_comparison(QP_result_temp-7),Bitrate_condition);
        end
        QP_result_final = QP_result_temp + 1;
        break;
    end
...
%     searching_flag = 0;
    ...
    % from 8, this case should be dealt with.
